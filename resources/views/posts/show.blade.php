            ...
            @auth
                @if($post->user_id === auth()->user()->id)
                    <form action="{{route('posts.destroy', $post)}}" method="POST">
                        @method('DELETE') 
                        @csrf
                        <input type="submit" value="Delete Post" class="bg-red-500 hover:bg-red-600 p-2 rounded text-white cursor-pointer font-bold mt-4"/>
                    </form>
                @endif
            @endauth
        </div>
        <div class="md:w-1/2 p-5">
            <div class="shadow bg-white p-5 pb-5">
            ...
            @auth
                    ...
                    @endif
                    <form 
                    action="{{ route('comentarios.store', ['user' => $user, 'post' => $post ]) }}" method="POST">
                    @csrf
                        <div class="mb-5">
                ...
                <div class="bg-white shadow mb-5 max-h-96 overflow-y-scroll mt-10">
                    @if ($post->comentarios->count())
                        ...
                        @endforeach
                    @else
                        <p class="p-10 text-center">There's no comments yet</p>
                    @endif
                </div>
            </div>
        </div>
